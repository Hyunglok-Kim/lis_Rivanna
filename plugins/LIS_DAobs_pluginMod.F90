...
#if ( defined DA_OBS_CYGNSSSM )
    use CYGNSSsm_Mod,            only : CYGNSSsm_setup
#endif
...
#if ( defined DA_OBS_CYGNSSSM )
    external read_CYGNSSsm, write_CYGNSSsmobs
#endif
...
#if ( defined DA_OBS_CYGNSSSM )
   call registerdaobsclass(trim(LIS_CYGNSSsmobsId),"LSM")
   call registerdaobssetup(trim(LIS_CYGNSSsmobsId)//char(0),&
        CYGNSSsm_setup)
   call registerreaddaobs(trim(LIS_CYGNSSsmobsId)//char(0),&
        read_CYGNSSsm)
   call registerwritedaobs(trim(LIS_CYGNSSsmobsId)//char(0),&
        write_CYGNSSsmobs)
#endif
